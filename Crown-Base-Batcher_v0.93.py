"""
Crown-Base Batcher 0.93
by teon-u (github)
update date : 2023-12-115
[Update Log] 0.9 > 0.91
...
[추후 필수 업데이트]
- tkinker 변수 활용 가능하게끔 설정 (외부변수 함수 내부로 호출)
    ...
    for idx, implant in enumerate(implants):
        ...
        for y in range(base_height):
            ...
            if placed:
                break
        if not placed:
            print("      크라운 배치 실패 : Crown No.", idx + 1,"/",implant_count)#, "Name:", implant_image_paths[idx][46:-23])
...
def execute_action(var_desc, var_size, var_rotate):
    ...
    """
    print("    [Crown-Base Batcher 0.93 by teon-u]")
    print("    Options")
    print("    ㄴ 배치순서 :",bool(var_desc)," - True 면 큰것부터, False 면 작은것부터")
    ...
    image_size_and_paths.sort(key=lambda x: x[0][0] * x[0][1], reverse=bool(var_desc))#True)
...
# 윈도우 제목 설정
root.title("Crown-Base Batcher v0.93")
# 중의적 의미 ㅋㅋ 배치 - Batch
